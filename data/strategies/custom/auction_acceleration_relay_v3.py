"""竞价加速接力模型3.0 - 非小微盘日线可回测近似版。

对应原模型自由流通市值大于等于 28 亿元的分池。当前 enriched 日线数据缺少真实
竞价量比时，使用日线成交额、5 日量比和换手率近似非小微盘的合力要求。
"""
import polars as pl
...
META = {
    "id": "auction_acceleration_relay_v3",
    "name": "竞价加速接力模型3.0（非小微盘）",
    "description": "自由流通市值不低于28亿元，在接力情绪活跃时筛选竞价量能合力与开盘继续加速的标的",
    "tags": ["竞价", "连板", "接力", "超短", "非小微盘"],
    "version": "1.1.0",
    "basic_filter": {
        "price_min": 3,
        "price_max": 120,
        "market_cap_min": 10e8,
        "float_cap_min": SMALL_CAP_BOUNDARY,
        "float_cap_max": None,
        "amount_min": 1.5e8,
        "exclude_st": True,
        "exclude_new_days": 60,
    ...
    "params": [
        ...
        {"id": "min_vol_ratio", "label": "最低5日量比（竞价量比代理）", "type": "float",
         "default": 1.2, "min": 0.5, "max": 5.0, "step": 0.1},
        {"id": "max_vol_ratio", "label": "最高5日量比", "type": "float",
        ...
        {"id": "max_turnover", "label": "最高换手率%", "type": "float",
         "default": 28.0, "min": 5.0, "max": 60.0, "step": 1.0},
        ...
        {"id": "min_market_limit_up_count", "label": "信号日全市场最低涨停家数", "type": "int",
         "default": 60, "min": 0, "max": 200, "step": 5},
...
ENTRY_SIGNALS = ["signal_entry_auction_acceleration_relay_v3"]
EXIT_SIGNALS = [
    "signal_exit_next_day_not_accelerating_v3",
    "signal_exit_acceleration_break_v3",
    "signal_ma20_breakdown",
]
STOP_LOSS = -0.06
MAX_HOLD_DAYS = 3
ALERTS = [
    {"field": "signal_entry_auction_acceleration_relay_v3", "message": "竞价加速接力模型3.0命中非小微盘接力候选"}
]

RULES = """
1. 自由流通市值不低于28亿元，对应原模型的非小微盘分池。
2. 前一交易日需要涨停或处于1至3连板，近10日涨幅不低于15%。
3. 当日开盘涨幅需要高于前一交易日开盘涨幅，近似竞价继续加速。
4. 非小微盘必须具备更强量能，使用日成交额、5日量比和换手率近似竞价量比不低于3%。
5. 收盘位置需要靠近日内高位，过滤竞价强而盘口承接不足的假强。
6. 次日只有一字板、强高开后秒板或继续涨停加速才继续持有。
7. 次日小高开、低开、冲高乏力、断板或不能继续加速时，触发竞价或早盘处理的卖出信号。
8. 断板后不做反包格局，断板卖出信号优先于继续持有。
9. 信号日全市场涨停家数少于60家时暂停新增仓位，过滤接力情绪不足的交易日。
10. 涨停家数过滤依赖完整市场数据，历史回测需要使用全市场股票范围。
"""
...
def filter_history(df: pl.DataFrame, params: dict) -> pl.DataFrame:
    ...
    max_vol_ratio = float(params.get("max_vol_ratio", 5.0))
    # turnover_rate 在 enriched 数据中使用百分数值，例如 8.5 表示 8.5%。
    min_turnover = float(params.get("min_turnover", 3.0))
    max_turnover = float(params.get("max_turnover", 28.0))
    min_close_strength = float(params.get("min_close_strength", 0.65))
    min_market_limit_up_count = int(params.get("min_market_limit_up_count", 60))
    ...
    # 日线量比不是竞价成交额占比；非小微盘使用更高量比与成交额要求近似合力。
    volume_confirm = (
    ...
    hist = hist.with_columns([
        ...
        entry_signal.alias("signal_entry_auction_acceleration_relay_v3"),
    ])

    hist = hist.with_columns([
        pl.col("signal_entry_auction_acceleration_relay_v3")
        .shift(1)
        .over("symbol")
        .fill_null(False)
    ...
    hist = hist.with_columns([
        acceleration_break.alias("signal_exit_acceleration_break_v3"),
        (pl.col("_entry_signal_t1") & ~next_continue_accelerating)
        .alias("signal_exit_next_day_not_accelerating_v3"),
    ])

    return hist.filter(
        pl.col("signal_entry_auction_acceleration_relay_v3")
        | pl.col("signal_exit_next_day_not_accelerating_v3")
        | pl.col("signal_exit_acceleration_break_v3")
    )
